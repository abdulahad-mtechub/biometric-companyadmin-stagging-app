import AbsenceListModal from '@components/CustomModal/AbsenceListModal';
...
export default function AbsenceCalendar({
  ...
  const fetchDailyAbsences = async date => {
    try {
      ...
      logger.error('❌ Error fetching daily absences:', error, {
        context: 'AbsenceCalendar',
      });
      setDailyAbsences([]);
    } finally {
  ...
  const handlePressEvent = event => {
    ...
    if (event.isGroup) {
      logger.log('Group absences:', event.absences, {
        context: 'AbsenceCalendar',
      });
