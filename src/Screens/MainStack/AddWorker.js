...
import {useDepartments} from '../../utils/Hooks/Hooks';
...
const useFormValidation = (formData, t, showAlert) => {
  ...
  const validateAddressDetails = useCallback(
    (setErrors, isManualAddress) => {
      const {address, city, state, postalCode, country, longitude, latitude} =
        formData;

      let errors = {};
      if (isManualAddress) {
        if (!country.label) errors.country = t('Please select country.');
        if (!state.label) errors.state = t('Please select state.');
        if (!city.label) errors.city = t('Please select city.');
...
const useLocationData = (formData, currentStep) => {
  ...
  useEffect(() => {
    const loadCountries = async () => {
      try {
        ...
        logger.error('Error loading countries:', error, {context: 'AddWorker'});
      }
    };
    loadCountries();
  }, []);
  ...
  useEffect(() => {
    ...
    const loadStates = async () => {
      try {
        ...
        logger.error('Error loading states:', error, {context: 'AddWorker'});
      }
    };
    loadStates();
  ...
  useEffect(() => {
    ...
    const loadCities = async () => {
      try {
        ...
        logger.error('Error loading cities:', error, {context: 'AddWorker'});
      }
    };
    loadCities();
...
const AddWorker = ({navigation, route}) => {
  ...
  const locationDropdownData = useLocationData(formData, currentStep);
...
const getCurrentPosition = useCallback(async () => {
  ...
  const handleImagePick = useCallback(
    img => {
      logger.log(selectImageType, {context: 'AddWorker'});
      if (selectImageType === 'profile') {
        ...
        // Handle both image and document from CameraBottomSheet
  ...
  const uploadFileToServer = useCallback(
    async (file, type = 'image') => {
      ...
      try {
        ...
        logger.error(`${type} upload failed:`, error, {context: 'AddWorker'});
        return null;
      }
    },
    [baseUrl],
  ...
  const handleBack = useCallback(() => {
    const stepMap = {
      ...
      [SCREEN_STEPS.EMPLOYMENTDETAILS]: () => {
        setSelectImageType('profile'); // Set for profile image upload in personal details
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!connected) {
      ...
      const result = await savePendingAction('CREATE_WORKER', {
        url: `${baseUrl}/company-admins/workers`,
        data: payload,
        token,
        type: 'worker',
      });
  ...
  }, [formData, token, showAlert, uploadFileToServer]);
    ...
    const stepActions = {
      ...
      [SCREEN_STEPS.PERSONAL_DETAILS]: () => {
        ...
        if (validatePersonalDetails(setPersonalDetailsErrors)) {
          setSelectImageType('nationalID'); // Set for national ID upload in employment details
          setStep(2);
          setCurrentStep(SCREEN_STEPS.EMPLOYMENTDETAILS);
        }
      },
  ...
  useEffect(() => {
    const checkOnlineAndGetPosition = async () => {
      ...
      <View style={styles.contentContainer}>
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.subheading}>
            ...
            {t('Tell us the personal details of employee')}
          </Text>
        </View>
  ...
  const viewSelectedDocument = useCallback(uri => {
    logger.log('Attempting to view document:', {uri, context: 'AddWorker'});
    viewDocument({
      uri: uri,
      mimeType: 'application/pdf',
    }).catch(error => {
      logger.error('Document viewer error:', error, {context: 'AddWorker'});
      logger.log('Document URI that failed:', uri, {context: 'AddWorker'});
        ...
        <View style={{marginBottom: hp(2)}}>
          ...
          {formData.nationalID?.path ? (
            formData.nationalID.type === 'image' ? (
              <Image
                source={{uri: formData.nationalID?.path || formData.imageURL}}
                style={{height: hp(30), width: '100%', borderRadius: wp(2)}}
              />
            ) : formData.nationalID.type === 'document' ? (
              <TouchableOpacity
                style={styles.uploadContainer}
                onPress={() => viewSelectedDocument(formData.nationalID.path)}>
                <Svgs.pdf />
                <Text
                  style={[
                    styles.label,
  ...
  //     logger.log({updates}, {context: 'AddWorker'});
  //     Object.entries(updates).forEach(([key, value]) => {
  //       updateFormField(key, value);
  //     });
  //   } catch (error) {}
  // };
    ...
    setShouldDisplayListView(false)
  }, []);
  ...
  const renderAddressDetails = () => (
    <View style={[styles.inputsContainer ,  {marginBottom: 0, alignItems: undefined},]}>
      <View style={styles.headerContainer}>
        <Text
          style={[
            styles.heading,
  ...
  const renderButtons = () => {
    ...
    return (
      <View style={{flexDirection: 'row'}}>
        ...
        />
        <CustomButton
          text={isWorkerDetailsStep ? 'Confirm & Next' : 'Next'}
          onPress={handleContinue}
          textStyle={[
            styles.continueButtonText,
            language.value === 'es' && {fontSize: RFPercentage(1.5)},
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <CameraBottomSheet
        ...
        showDocument={selectImageType === 'nationalID'}
      />
