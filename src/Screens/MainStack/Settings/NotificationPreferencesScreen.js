// screens/NotificationPreferencesScreen.js
import React, { useState } from 'react';
import { useTranslation } from 'react-i18next';
import { ScrollView, StyleSheet, Text, View } from 'react-native';
import { RFPercentage } from 'react-native-responsive-fontsize';
import {
  heightPercentageToDP as hp,
  widthPercentageToDP as wp,
} from 'react-native-responsive-screen';
import { useSelector } from 'react-redux';
import LabeledSwitch from '@components/Buttons/LabeledSwitch';
import StackHeader from '@components/Header/StackHeader';
import { Fonts } from '@constants/Fonts';
import { pxToPercentage } from '@utils/responsive';
import logger from '@utils/logger';

const NotificationPreferencesScreen = ({navigation}) => {
  const {isDarkMode,Colors} = useSelector(store => store.theme);
  const styles = dynamicStyles(isDarkMode,Colors);
  const {t} = useTranslation();

  // === STATES ===
  const stateMap = {
    // Messages
    newGroupMsg: useState(false),
    directMsg: useState(false),
    mentionInGroup: useState(false),

    // Attendance
    clockInMethodChange: useState(false),
    shiftChange: useState(true),
    validPunch: useState(true),
    invalidPunch: useState(false),
    assignedZoneChange: useState(true),
    manualPunchChange: useState(true),

    // Task
    newTask: useState(false),
    overdueTask: useState(true),
    deadlineReminder: useState(true),
    mentionInComment: useState(false),
    docUpdated: useState(true),

    // Payments
    expenseApproved: useState(false),
    salaryTransfer: useState(true),
    loanApproved: useState(true),
    loanReminder: useState(false),
    remainingLoan: useState(false),

    // Request
    reqStatus: useState(false),
    reqRejected: useState(true),
    reqApproved: useState(true),

    // Document
    docDownloaded: useState(false),
    policyUpdated: useState(true),
    newPolicy: useState(true),

    // Other
    profileUpdated: useState(false),
    addedToDept: useState(true),
    addedToTeam: useState(true),
    promoted: useState(false),
  };

  const getSwitch = (label, stateKey) => {
    const [value, setter] = stateMap[stateKey];
    return {
      label: t(label),
      value,
      onToggle: () => setter(!value),
    };
  };

  const sections = [
    {
      title: t('Employee Management'),
      data: [
        getSwitch('New Request', 'newTask'),
        getSwitch('Account Verified', 'reqApproved'),
        getSwitch('Employee Account Deactivation/Deletion', 'reqRejected'),
      ],
    },
    {
      title: t('Attendance'),
      data: [
        getSwitch('Clock-In/Clock-Out', 'clockInMethodChange'),
        getSwitch('Break Start/Break Ends', 'shiftChange'),
        getSwitch('Invalid Punches', 'invalidPunch'),
        getSwitch('Early Out', 'mentionInGroup'),
        getSwitch('Early In', 'mentionInComment'),
        getSwitch('Late Arrival', 'validPunch'),
        getSwitch('Late Out', 'loanApproved'),
        getSwitch('Extra Working Hours', 'loanReminder'),
      ],
    },
    {
      title: t('Task'),
      data: [
        getSwitch('New Task Assigned', 'newTask'),
        getSwitch('Overdue Tasks', 'overdueTask'),
        getSwitch('Deadline Reminders', 'deadlineReminder'),
        getSwitch('Mentioning In Comments', 'mentionInComment'),
        getSwitch('Task Status Changes', 'docUpdated'),
        getSwitch('Document updated/uploaded in task', 'docUpdated'),
      ],
    },
    {
      title: t('Messages'),
      data: [
        getSwitch('New Message in Group', 'newGroupMsg'),
        getSwitch('Direct Message in from Inbox', 'directMsg'),
        getSwitch('Mentioning in Group', 'mentionInGroup'),
        getSwitch('Support Message', 'profileUpdated'),
        getSwitch('Account Executive Message', 'promoted'),
      ],
    },
    {
      title: t('Payments'),
      data: [
        getSwitch('Subscription Expired Reminder', 'expenseApproved'),
        getSwitch('Trial Ended', 'salaryTransfer'),
        getSwitch('Receive Loan Instalment', 'loanApproved'),
        getSwitch('Loan Instalment Reminders', 'loanReminder'),
        getSwitch('Remaining Loan Amount', 'remainingLoan'),
      ],
    },
    {
      title: t('Request'),
      data: [
        getSwitch('New Request Receive', 'reqStatus'),
        getSwitch('Pending Request', 'reqApproved'),
      ],
    },
    {
      title: t('Document'),
      data: [
        getSwitch('When Someone Download Company Policies', 'docDownloaded'),
        getSwitch('Employee Updates Document', 'policyUpdated'),
        getSwitch('Employee Uploads Document', 'newPolicy'),
      ],
    },
    {
      title: t('Other Notifications'),
      data: [
        getSwitch('Profile Verified by Admin', 'newGroupMsg'),
        getSwitch('Profile Rejected by Admin', 'reqRejected'),
        getSwitch('Comment by Admin', 'mentionInComment'),
      ],
    },
  ];
  

  const NotificationSection = ({title, data}) => (
    <View>
      <Text style={styles.sectionTitle}>{title}</Text>
      <View style={styles.sectionContainer}>
        {data.map((item, index) => (
          <LabeledSwitch
            key={index}
            title={item.label}
            value={item.value}
            onValueChange={item.onToggle}
          />
        ))}
      </View>
    </View>
  );

  return (
    <View style={styles.container}>
      <StackHeader
        title={t('Notification Preferences')}
        headerTxtStyle={{
          textAlign: 'left',
          fontSize: RFPercentage(2.5),
          fontFamily: Fonts.PoppinsSemiBold,
        }}
        headerStyle={{paddingVertical: hp(2), backgroundColor: isDarkMode ? Colors.darkTheme.secondryColor : Colors.lightTheme.backgroundColor}}
        onBackPress={() => navigation.goBack()}
      />
      <ScrollView
        style={styles.scrollContainer}
        showsVerticalScrollIndicator={false}
        contentContainerStyle={{paddingBottom: hp(5)}}>
        {sections.map((section, index) => (
          <NotificationSection
            key={index}
            title={section.title}
            data={section.data}
          />
        ))}
      </ScrollView>
    </View>
  );
};

export default NotificationPreferencesScreen;

const dynamicStyles = (isDarkMode,Colors) =>
  StyleSheet.create({
    container: {
      flex: 1,
      backgroundColor: isDarkMode
        ? Colors.darkTheme.backgroundColor
        : Colors.lightTheme.secondryColor,
    },
    scrollContainer: {
      flex: 1,
      paddingHorizontal: wp(5),
      marginTop: 10,
    },
    sectionTitle: {
      fontFamily: Fonts.PoppinsMedium,
      fontSize: RFPercentage(pxToPercentage(16)),
      color: isDarkMode
        ? Colors.darkTheme.primaryTextColor
        : Colors.lightTheme.secondryTextColor,
      marginBottom: hp(1),
    },
    sectionContainer: {
      marginBottom: hp(3),
      backgroundColor: isDarkMode
        ? Colors.darkTheme.secondryColor
        : Colors.lightTheme.backgroundColor,
      borderRadius: 12,
      paddingHorizontal: wp(2),
      paddingVertical: hp(2),
    },
  });
