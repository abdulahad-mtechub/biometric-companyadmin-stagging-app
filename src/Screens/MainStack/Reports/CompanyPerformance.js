...
const CompanyPerformance = ({dateStart, dateEnd}) => {
  ...
  const previewData = useMemo(() => {
    return [
      { label: t("Company Name"), value: apiData?.company_name },
      { label: t("Period Start"), value: new Date(apiData?.period_start).toLocaleDateString("en-GB", { day: '2-digit', month: 'short', year: 'numeric' }) },
      { label: t("Period End"), value: new Date(apiData?.period_end).toLocaleDateString("en-GB", { day: '2-digit', month: 'short', year: 'numeric' }) },
      { label: t("Tasks Assigned"), value: apiData?.tasks_assigned },
      { label: t("Tasks Completed"), value: apiData?.tasks_completed },
      { label: t("Productive Tasks"), value: apiData?.productive_tasks },
      { label: t("Average Task Time (Hours)"), value: apiData?.average_task_time_hours },
      { label: t("Total Hours Worked"), value: apiData?.total_hours_worked },
      { label: t("Employees with Tasks"), value: apiData?.workers_with_tasks },
      { label: t("Employees with attendance"), value: apiData?.workers_with_attendance },
      { label: t("Working Days"), value: apiData?.working_days },
      { label: t("Avg Daily Hours per Employee"), value: apiData?.avg_daily_hours_per_worker },
      { label: t("Compliance Percentage"), value: apiData?.compliance_percentage },
      { label: t("Productivity Percentage"), value: apiData?.productivity_percentage },
      { label: t("Efficiency Percentage"), value: apiData?.efficiency_percentage }
    ];;
  }, [apiData]);
  // ---------- Bar Chart Data ----------
  ...
  return (
    <View>
      <View style={styles.rowViewSB}>
        <Text style={styles.Heading}>{t('Company Performance')}</Text>
      </View>
      {/* ----------- Card 1: Bar Chart ----------- */}
      {isLoading ? (
        <Loader size={wp(10)} />
