...
const GlobalSearch = ({navigation}) => {
  ...
  const SectionBlock = ({title, data, nameKey, statusKey}) => (
    <View style={styles.logsContainer}>
      ...
      <View style={[styles.rowSb, styles.workerStatusContainer]}>
        <Text style={styles.SubHeading}>{t("Name")}</Text>
    ...
    <View style={styles.container}>
      ...
      filteredCommission.length === 0 ? (
        ...
        <View style={styles.noResultContainer}>
          <Svgs.NoResult />
          <Text style={styles.noResultTitle}>{t("No Result Found!")}</Text>
          <Text style={styles.noResultSubtitle}>
            {t("Please try searching using alternative keywords.")}
          </Text>
        </View>
        </View>
      ) : (
